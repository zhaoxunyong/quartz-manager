...
public class LL_ws {
	...
	public JSONArray getUnreadMsg(int userId) {
		...
		JSONArray getJsonArray = JSON.parseArray(msg);
		...
		for (int i = 0; i < getJsonArray.size(); i++) {
			...
			toMessage.put("avatar",
					getJsonObj.getJSONObject("friend_message").getJSONObject("mine").getString("avatar"));
			toMessage.put("type", type);
			toMessage.put("content",
					getJsonObj.getJSONObject("friend_message").getJSONObject("mine").getString("content"));
			toMessage.put("timestamp", getJsonObj.getJSONObject("friend_message").getString("time"));
			toMessage.put("username",
					getJsonObj.getJSONObject("friend_message").getJSONObject("mine").getString("username"));
			toMessage.put("mine", false);
			if (type.equals("friend") || type.equals("fankui")) {
				toMessage.put("id", getJsonObj.getJSONObject("friend_message").getJSONObject("mine").getIntValue("id")); // 如果是私聊，则是用户id
			} else {
				toMessage.put("id", getJsonObj.getJSONObject("friend_message").getJSONObject("to").getIntValue("id")); // 如果是群聊，则是群组id
			}
	...
	public JSONArray getUnreadQunMsg(int userId) {
		...
		JSONArray getJsonArray = JSONArray.parseArray(msg);
		JSONArray toJsonArray = new JSONArray();
		for (int i = 0; i < getJsonArray.size(); i++) {
			JSONObject getJsonObj = getJsonArray.getJSONObject(i);
			JSONObject toMessage = new JSONObject();
			...
			toMessage.put("mine", false);
			toMessage.put("id", getJsonObj.getJSONObject("offline_message").getIntValue("id")); // 如果是群聊，则是群组id
			toJsonArray.add(toMessage);
		...
		if (map.containsValue(session)) {
			for (Entry<String, Session> entry : map.entrySet()) {
				if (entry.getValue().equals(session)) {
					subOnlineCount(Integer.parseInt(entry.getKey())); // 在线数减1，
																		// 更改数据库用户在线状态为0，不在线。
	...
	public void onMessage(String message, Session session) {
		...
		case "fankui": // 家长与老师反馈,记录到mongo
			...
			break;
		case "group":
			JSONArray memberList = JSON.parseArray(llClient.getGroupUser(toId).toString()); // 获取在线用户
			llClient.saveGroupMessage(jsonObject);
			if (memberList.size() > 0) {
				for (int i = 0; i < memberList.size(); i++) { // 发送到在线用户(除了发送者)
					if (map.containsKey(memberList.get(i))
							&& !memberList.get(i).equals(jsonObject.getJSONObject("mine").getIntValue("id") + "")) {
					...
					} else if (memberList.get(i).equals(jsonObject.getJSONObject("mine").getIntValue("id") + "")) {
						// 如果是发送者自己，不做任何操作。
