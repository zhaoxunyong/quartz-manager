...
import com.woyi.mhub.im.domain.Friend;
import com.woyi.mhub.im.domain.FriendType;
import com.woyi.mhub.im.domain.SNSFriend;
import com.woyi.mhub.im.domain.SNSGroup;
import com.woyi.mhub.im.domain.SNSInit;
import com.woyi.mhub.im.domain.SNSUser;
import com.woyi.mhub.im.domain.SNSdata;
import com.woyi.mhub.im.domain.User;
import com.woyi.mhub.im.domain.UserInfo;
import com.woyi.mhub.im.service.FriendService;
import com.woyi.mhub.im.service.FriendTypeService;
import com.woyi.mhub.im.service.GroupUserService;
import com.woyi.mhub.im.service.UserInfoService;
import com.woyi.mhub.im.service.UserOnlineService;
import com.woyi.mhub.im.service.UserService;
...
public class FriendWS {
	...
	public String getHistoryMessageFromMongo(HttpSession session, int id, String type, Model model) {
		...
		if (type.equals("friend")) {
			...
			List<Document> pipeline = Arrays.asList(new Document("$sort", sortFields1), new Document("$match", grep),
					new Document("$limit", pageSize), new Document("$sort", sortFields2));
	...
	public @ResponseBody String getHistoryMessageFromMongo(HttpSession session, int id, String type, Integer pageNum) {
		...
		if (type.equals("friend")) {
			...
			Document sortFields1 = new Document("_id", -1);
			Document sortFields2 = new Document("_id", 1);
			List<Document> pipeline = Arrays.asList(new Document("$sort", sortFields1),
					new Document("$skip", (pageNum - 1) * pageSize), new Document("$limit", pageSize),
					new Document("$sort", sortFields2));
			List<Document> results = friendMessages.aggregate(pipeline).into(new ArrayList<Document>());
			for (Document cur : results) {
				s += cur.toJson() + ',';
			}
	...
	public @ResponseBody String getUnreadMessage(int userId) {
		...
		MongoDatabase md = mu.getDataBase(MongoUtils.DATABASE);
		MongoCollection<Document> friendMessages = md.getCollection(MongoUtils.FRIEND_MESSAGE);
		FindIterable<Document> findIterable = friendMessages
				.find(new Document("friend_message.to.id", userId).append("friend_message.mStatus", 0));
		String s = "";
		MongoCursor<Document> mongoCursor = findIterable.iterator();
		while (mongoCursor.hasNext()) {
			s += mongoCursor.next().toJson() + ',';
	...
	public @ResponseBody Page<User> searchFriend(String searchStr, String userRole, String provinceCode,
			String cityCode, String countyCode, Integer schoolId, String gradeCode, Integer classId, Integer pageNum,
			Integer pageSize) {
		// Calendar cal = Calendar.getInstance();
		// String schoolYear = cal.get(Calendar.YEAR)+"";
		String schoolYear = "2015";
		Page<User> user = userService.search(searchStr, userRole, schoolYear, provinceCode, cityCode, countyCode,
				schoolId, gradeCode, classId, pageNum, pageSize);
		return user;
	}

	/*
	 * @author jack 给新版聊聊提供初始化数据服务，包括个人信息、好友列表信息、群组列表信息
	 * 
	 * @param userId
	 */
